    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [
                ...
                supabase.from('transactions').select('amount').eq('type', 'expense').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).or('payment_method.neq.transferencia,payment_method.is.null').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                ...
                supabase.from('transactions').select('amount').eq('type', 'income').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).or('payment_method.neq.transferencia,payment_method.is.null').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('type', 'expense').eq('is_business', isBusiness).eq('status', 'open').is('transfer_id', null).is('investment_id', null).or('payment_method.neq.transferencia,payment_method.is.null').eq('due_date', todayStr).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                ...
                supabase.from('transactions').select('*, categories(name, icon, color), accounts:accounts!transactions_account_id_fkey(name)').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).or('payment_method.neq.transferencia,payment_method.is.null').order('date', { ascending: false }).limit(5).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
