    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [
                ...
                supabase.from('investments').select('*').eq('is_business', isBusiness).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('type', 'expense').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount, category_id, categories(id, name, color, parent_id)').eq('type', 'expense').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('categories').select('id, name, color, parent_id').eq('is_business', isBusiness).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('type', 'income').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('type', 'expense').eq('is_business', isBusiness).eq('status', 'open').is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').eq('due_date', todayStr).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('cards').select('credit_limit').eq('is_business', isBusiness).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('payment_method', 'credito').eq('is_business', isBusiness).eq('status', 'open').is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('amount').eq('payment_method', 'credito').eq('is_business', isBusiness).eq('status', 'open').is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').gte('due_date', formatDate(startOfMonth)).lte('due_date', formatDate(endOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('transactions').select('*, categories(name, icon, color), accounts:accounts!transactions_account_id_fkey(name)').eq('is_business', isBusiness).is('transfer_id', null).is('investment_id', null).neq('payment_method', 'transferencia').order('date', { ascending: false }).limit(5).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null),
                supabase.from('budgets').select('*, categories(name, color, icon)').eq('is_business', isBusiness).eq('month', formatDate(startOfMonth)).filter('company_id', isBusiness ? 'eq' : 'is', isBusiness ? activeCompany?.id : null)
            ]));
