...
const Transactions: React.FC = () => {
  ...
  const totalIncome = transactions.filter(t => t.type?.toLowerCase() === 'income' && !t.transfer_id && !t.investment_id && t.payment_method?.toLowerCase() !== 'transferencia' && t.type?.toLowerCase() !== 'transfer' && t.type?.toLowerCase() !== 'investment').reduce((acc, t) => acc + Number(t.amount), 0);
  const totalExpense = transactions.filter(t => t.type?.toLowerCase() === 'expense' && !t.transfer_id && !t.investment_id && t.payment_method?.toLowerCase() !== 'transferencia' && t.type?.toLowerCase() !== 'transfer' && t.type?.toLowerCase() !== 'investment').reduce((acc, t) => acc + Number(t.amount), 0);
