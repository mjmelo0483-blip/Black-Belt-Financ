          ...
          {hierarchy.length > 0 ? (
            hierarchy.map((root) => (
              <React.Fragment key={root.id}>
                {/* Parent */}
                <div className="bg-[#1c2a38]/80 backdrop-blur-xl border border-[#324d67]/50 rounded-xl overflow-hidden hover:border-primary/50 transition-all group shadow-sm mb-3">
                  <div className="flex items-center justify-between p-4 cursor-pointer" onClick={() => handleEditClick(root)}>
                    <div className="flex items-center gap-4">
                      <div className="size-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ backgroundColor: root.color }}>
                        <span className="material-symbols-outlined">{root.icon || 'label'}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-white font-bold">{root.name}</span>
                        <span className="text-[10px] uppercase font-black tracking-widest" style={{ color: root.color }}>
                          {root.type === 'income' ? 'Receita' : 'Despesa'}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => { e.stopPropagation(); handleEditClick(root); }} className="p-2 text-[#92adc9] hover:text-white hover:bg-[#233648] rounded-lg transition-all"><span className="material-symbols-outlined text-[20px]">edit</span></button>
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(root.id); }} className="p-2 text-[#92adc9] hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"><span className="material-symbols-outlined text-[20px]">delete</span></button>
                    </div>
                  </div>
                </div>
                ...
                {root.children.length > 0 && (
                  <div className="ml-8 pl-4 border-l-2 border-[#233648] space-y-3 mb-6">
                    {root.children.map((child: any) => (
                      <div key={child.id} className="bg-[#1c2a38]/80 backdrop-blur-xl border border-[#324d67]/50 rounded-xl overflow-hidden hover:border-primary/50 transition-all group shadow-sm">
                        <div className="flex items-center justify-between p-3 cursor-pointer" onClick={() => handleEditClick(child)}>
                          <div className="flex items-center gap-3">
                            <div className="size-8 rounded-lg flex items-center justify-center text-white shadow-lg opacity-80" style={{ backgroundColor: child.color }}>
                              <span className="material-symbols-outlined text-sm">{child.icon || 'label'}</span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-white text-sm font-bold">{child.name}</span>
                              <span className="text-[10px] text-[#92adc9]">Subcategoria de {root.name}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={(e) => { e.stopPropagation(); handleEditClick(child); }} className="p-1.5 text-[#92adc9] hover:text-white hover:bg-[#233648] rounded-lg"><span className="material-symbols-outlined text-[18px]">edit</span></button>
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(child.id); }} className="p-1.5 text-[#92adc9] hover:text-red-500 hover:bg-red-500/10 rounded-lg"><span className="material-symbols-outlined text-[18px]">delete</span></button>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="py-20 text-center bg-[#1c2a38]/50 rounded-2xl border-2 border-dashed border-[#324d67]/50">
              ...
              <p className="text-[#92adc9]">Nenhuma categoria cadastrada.</p>
            </div>
          )}
        </div>
